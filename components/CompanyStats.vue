<template>
    <div class="s">
    <!-- <div class="secstion  wf-section"> -->
      <!-- <img src="images/Rectangle-240-copy.svg" loading="lazy" alt="" class="absolute-image"> -->
    <div class="container">
      <div class="spacer _32"></div>
      <div class="_12-columns align-center">
        <!-- <div class="column desk-4">
          <div class="text-box">
            <h2 class="header-large">10</h2>
            <p>Prebuilt framework starters</p>
          </div>
          <div class="spacer _32"></div>
        </div> -->
        <div class="column desk-4">
          <div class="text-box space-y-1">
            <h2 class="header-large">5,200+</h2>
            <p class="text-lg">Developers active this week</p>
          </div>
          <div class="spacer _32"></div>
        </div>
        <div class="column desk-4">
          <div class="text-box space-y-1">
            <h2 class="header-large">101,100+</h2>
            <p class="text-lg">Developers on Cyclic</p>
          </div>
          <div class="spacer _32"></div>
        </div>
        <div class="column desk-4">
          <div class="text-box space-y-1">
            <h2 class="header-large">1,160,000+</h2>
            <p class="text-lg">Builds triggered</p>
          </div>
          <div class="spacer _32"></div>
        </div>
      </div>
    </div>
  </div>
</template>
<style scoped>
.s{
  background: linear-gradient(
      to bottom,
      rgba(0, 0, 0, 0) -21%,
      var(--background-color)
    ),
    var(--background-pattern-dark) !important;
  border-radius: 8px;;
}
h2{
  font-weight: bold;
}
</style>
